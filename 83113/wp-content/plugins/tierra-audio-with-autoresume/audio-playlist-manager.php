<?php
/*
 * Plugin Name: Tierra Audio with Autoresume
 * Plugin URI: http://www.toughride.com/wordpress/how-to-embed-mp3-to-a-website-and-play-continuously-without-interruption-across-pages/
 * Description: Enhanced plugin of the original 'Tierra Audio Playlist Manager' with 'Autoresume' Features
 * Version: 2.2.3
 * Author: Toughride
 * Author URI: http://www.toughride.com/
 * 
 */
/*
 ...
 * GPL2 License: http://www.gnu.org/licenses/old-licenses/gpl-2.0.txt
 */
/*
Changes:
 2.2.3	- Changed links to license agreements
 2.2.2 	- Fixed invisible player bug
 
 2.2.1 	- Made autoload playlist prior to user interaction; regardless of 'autoplay' status.
...
wp_enqueue_script ('ac_run_active_content', WP_PLUGIN_URL . "/tierra-audio-with-autoresume/js/AC_RunActiveContent.js");
...
function ti_apm_widget_output_control() {
	global $wpdb;
	//update_option ("ti_apm_widget_settings", array('autoplay' => true, 'autoresume' => true, 'repeat'=>true, 'random' => true, title=> 'Test', id=>'200'));
	if (intval( $_POST['ti_apm_widget_controller_submit'] ) == 1)	{
		...
		$ar = isset($_POST['ti_apm_autoresume_checkbox']) ? ti_apm_admin_booleanValue($_POST['ti_apm_autoresume_checkbox']) : false;
		...
		$array =  array('autoplay' => $ap, 'autoresume' => $ar, 'repeat'=>$repeat, 'random' => $random, 'title'=> $title, 'id'=>$id, 'volume'=>$volume, 'width'=>$width, 'height'=>$height, 'skin'=>$skin);
	...
	$cb_resume = ($opts[autoresume] == true) ? "CHECKED" : "";
	...
	$cb_random = ($opts[random] == true) ? "CHECKED" : "";
	 ...
	 print <<<__END_OF_FORM__
		...
		<p><input type='checkbox' name='ti_apm_autoresume_checkbox' id='ti_apm_autoresume_checkbox' $cb_resume /> Resume Playing after pageload </p>
		...
		if ($rows) {	
			foreach ($rows as $row)	{
				...
				print "<tr class='alternate author-self status-publish' valign='top'>
					...
					class='thickbox' href='". WP_PLUGIN_URL .	"/tierra-audio-with-autoresume/preview.php?media_id="
...
function ti_apm_return_playlist_xml($ti_apm_playlist_id)	{
	...
	header('Location: ' . WP_PLUGIN_URL . '/tierra-audio-with-autoresume/playlist.php?id=' . $ti_apm_playlist_id);
	exit;
}
			...
			if ($row) {
				$metadata = get_post_meta($row->id, '_wp_attachment_metadata', true);
					print "<tr class='alternate author-self status-publish' valign='top'>
						...
						<td scope='col'>" . htmlspecialchars(stripslashes($metadata['_ti_apm_album'])) . "</td>
<td><a name=\"Audio: " . htmlspecialchars(stripslashes($row->track)) . "\" id=\"Audio: " .  htmlspecialchars(stripslashes($row->track)) . "\"
						class='dynamicthickbox thickbox' href='". WP_PLUGIN_URL .	"/tierra-audio-with-autoresume/preview.php?media_id="
						. 	$wpdb->escape($row->id) . "&keepThis=true&TB_iframe=true&height=" .$ti_apm_prev_height . "&width=" . $ti_apm_prev_width .
						"'>Preview/Embed</a></td>
<!--						<td scope='col'><a href='" . $wpdb->escape($row->guid) ."' target='_blank'>" . $wpdb->escape($row->guid) ."</a></td> -->
	...
	print "<tr><th colspan='2'><a target='_blank' href='" . WP_PLUGIN_URL ."/tierra-audio-with-autoresume/playlist.php?preview=true&id=$ti_apm_playlist_id' title=''>Download XML File</a> (Right-click and copy URL to use elsewhere)";
	print "<th colspan='5'>";
	...
	print '<a class="dynamicthickbox thickbox alignright" name="Tracklist: ' . htmlspecialchars($tracklist->title) . '" id="Tracklist: ' . htmlspecialchars($tracklist->title) . '" href="'
			.  	WP_PLUGIN_URL 
			.	'/tierra-audio-with-autoresume/preview.php?name='
...
function ti_apm_print_player ($atts)	{
	...
	extract(shortcode_atts(array(  
		...
		"media" => "",
		"id" => 1,
		"skin" => WP_PLUGIN_URL . "/tierra-audio-with-autoresume/swf/xspf_player.swf",
		"autoplay" => 0,
		"autoresume" => 0,
		"autoload" => 1,
		"volume" => 50,
	...
	$playlistURL = WP_PLUGIN_URL . "/tierra-audio-with-autoresume/playlist.php?id=$ti_apm_playlist_id&media_id=$media_id&random=$randomize";
	
	
	$player =  preg_replace('/\.swf$/', '', $skin);
	$playerURL = $player . '.swf';
	
	$acURL =  WP_PLUGIN_URL ."/tierra-audio-with-autoresume/js/AC_RunActiveContent.js";
	$flashvars = ($autoplay ? "autoplay=" . urlencode($autoplay) : "")
				. ($autoresume ? "&autoresume=" . urlencode($autoresume) : "")
...
add_action('the_content_rss', 'ti_apm_remove_scriptcode_from_content_rss');
add_action('the_excerpt_rss', 'ti_apm_remove_scriptcode_from_excerpt_rss');
