...
WebFontConfig = {
    google: { families: [ 'Cabin', 'Raleway:100' ] }
};
...
jQuery(window).one("load", function(){
	...
	jQuery("a.play-icon[href*='selfid']").not('.noColorBox').each(function(){
    	var dataRel = jQuery(this).attr('data-rel');
        var dataTitle = jQuery(this).attr('data-title');
        jQuery(this).colorbox({iframe:true, innerWidth:800, innerHeight:450, scrolling:false, inline:true, rel:dataRel, current:"{current} / {total}", onComplete: function(){ jQuery('#cboxTitle').text(dataTitle); if(!jQuery('.pix_slide').hasClass('stopped') && jQuery('.pix_slide').length ){ jQuery('.pix_slide').pixwall_delightStop(); } else { jQuery('.pix_slide').addClass('alreadyStopped'); } }, onClosed: function(){ jQuery('#cboxTitle').text(''); if(!jQuery('.pix_slide').hasClass('alreadyStopped') && jQuery('.pix_slide').length ){ jQuery('.pix_slide').pixwall_delightPlay(); } else { jQuery('.pix_slide').removeClass('alreadyStopped'); } } });
    });
...
jQuery(document).ready(function() {
    ...
    jQuery('.contactForm').each(function () {        
		...
		jQuery('input[type=submit]',t).click(function () {
			...
			jQuery.ajax({
				url: "http://jeninaandmike.com/83113/wp-content/themes/delight/scripts/mailer/mailer.php",
				type: "GET",
				data: data,     
				cache: false,
    ...
    jQuery('.contactForm').each(function () { 
        ...
        if(jQuery.isFunction(jQuery.fn.datepicker)){      
            var dates = jQuery( "#from, #to", t ).datepicker({
                ...
                numberOfMonths: 1,
                showOn: "both",
                buttonImage: "http://jeninaandmike.com/83113/wp-content/themes/delight/images/calendar.png",
...
jQuery(document).ready(function() {
    ...
    jQuery('a.pix_tips_ajax').each(function(){
        ...
        t.qtip({
            content: {
                text: '<img class="throbber" src="http://jeninaandmike.com/83113/wp-content/themes/delight/images/ajax-loader.gif" alt="Loading...">',
                ajax: {
                   url: t.attr('data-rel')
                }
            },
	...
	if(jQuery('div.isoFilter').length!=0){
        ...
        if(jQuery.isFunction(jQuery.fn.infinitescroll)) {
            wall.infinitescroll({
                ...
                loadingImg : 'http://jeninaandmike.com/83113/wp-content/themes/delight/images/loader.gif',
                loadingText : 'Loading...',
