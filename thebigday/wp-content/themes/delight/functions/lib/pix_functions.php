...
function pix_admin_enqueue_scripts() {
	...
	wp_enqueue_script('pix-scripts', get_template_directory_uri()."/functions/scripts/pix_scripts.js", array('jquery','jquery-ui-tabs'));
}
...
function get_pix_thumb($image_src, $thumb_size) {
	...
	if($url_thumb2==''){
		...
		$url_thumb2= get_template_directory_uri().'/scripts/timthumb'.$timthumb_cache.'.php?src='.pix_image_path($image_src).'&amp;h=50&amp;w=50';
	}
	return $url_thumb2;
}
...
function pix_enq_accordion($posts){
	...
	foreach ( $posts as $post ){
		if ( stripos($post->post_content, 'pix_accordion') ){
	...
	if ( $shortcode_found ){
		wp_enqueue_script("jquery-ui-core");
		wp_enqueue_script("ui-widget", get_template_directory_uri()."/scripts/jquery.ui.widget.js", array('jquery-ui-core'));
		wp_enqueue_script("ui-accordion", get_template_directory_uri()."/scripts/jquery.ui.accordion.js", array('ui-widget'));
	}
...
function pix_enq_tabs($posts){
	...
	if ( $shortcode_found ){
		wp_enqueue_script("jquery-ui-core");
		wp_enqueue_script("ui-widget", get_template_directory_uri()."/scripts/jquery.ui.widget.js", array('jquery-ui-core'));
		wp_enqueue_script("ui-tabs", get_template_directory_uri()."/scripts/jquery.ui.tabs.js", array('ui-widget'));
	}
...
function register_pix_datepciker() {
		wp_register_script("ui-widget", get_template_directory_uri()."/scripts/jquery.ui.widget.js", array('jquery-ui-core'));
...
function pix_enqueue_cssinclude() {
	...
	if( $current_user->display_name != 'pix_test' && get_pix_option('pix_css_inline') != 'true' ) {
		wp_register_style('css-include', get_template_directory_uri() . '/functions/includes/css_include.css', 'style');
	} elseif( $current_user->display_name == 'pix_test' && get_pix_option('pix_css_inline') != 'true' ) {
		$session = session_id();
		if (!file_exists($inc_dir . 'sessions/css_include_session_'.$session.'.css')) {
			wp_register_style('css-include', get_template_directory_uri() . '/functions/includes/css_include.css', 'style');
		} else {
			wp_register_style('css-include', get_template_directory_uri() . '/functions/includes/sessions/css_include_session_'.$session.'.css', 'style');
		}
	} else {
		wp_register_style('css-include', get_template_directory_uri() . '/style.php', 'style');
	}
