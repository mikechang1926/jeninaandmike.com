...
<?php $theme_data = get_theme_data(ABSPATH . 'wp-content/themes/'.basename(TEMPLATEPATH).'/style.css');
$pix_cur_version = $theme_data['Version'];
?>
<div id="pix_wrap">
...
<?php 
	...
	if(get_pix_option('pix_css_inline')!='true' && !is_writeable($css_file)) { ?>
<div id="message" class="error" style="padding:5px 10px; margin:0 5px 10px">
	...
	<div id="pix_credits">
    	<?php if(get_pix_option('pix_show_tweets')=="show") { ?><div id="twitter_update_list"><ul>
<?php
	$tweets = json_decode(pix_cache_tweets($user='pixedelic',$count='1000')); // get tweets and decode them into a variable
	$pix_tweets = array();
	$tweet_key = 0;

	foreach ($tweets as $value) {
		if (  substr($value->text,0,1) != '@' && preg_match("/delight/i", $value->text)) {
			array_push($pix_tweets, '<li>'.pix_url_2_link($value->text).' <a style="font-size:85%" href="http://twitter.com/pixedelic/statuses/'.$value->id_str.'" target="_blank">'.pix_compare_dates($value->created_at).' &rarr;</a></li>');
	...
	if(count($pix_tweets)==0){
		echo 'Sorry, no tweets available';
	} else {
		while($i<3){
...
?>        </ul></div><?php } ?>
    ...
    <div id="pix_header">
        ...
        <div id="pix_body">
            ...
            <div id="general-tab">
                <ul>
                    <li><a href="#admin-panel-tab">Admin panel</a></li>
                ...
                <form method="post">
                <div id="admin-panel-tab">
                    <div class="block_separator">
                        <label for="pix_show_tweets">Show the tweets in the admin panel<small>Tweets are also useful to know when a new version of Delight is ready</small></label>
                        <input type="hidden" value="0" name="pix_show_tweets">
                        <input type="checkbox" value="show" name="pix_show_tweets"<?php if(get_pix_option('pix_show_tweets')=="show") { ?> checked="checked"<?php } ?>>
                    </div><!-- .block_separator -->
